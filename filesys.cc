...
bool FileSystem::Create(char *name, int initialSize)
{
    ...
    //---use strtok to parse the path
    OpenFile *file_ = directoryFile;
    ...
    while(token != NULL){
        ...
        file_ = new OpenFile(sector);
        directory->FetchFrom(file_);
        token_prev = token;
        token = strtok(NULL,"/");//keep finding
    }
...
int FileSystem::CreateDir(char *name)
{
    ...
    //DEBUG(dbgFile, "Creating file(dir) " << name << " size " << initialSize);
...
bool FileSystem::recurRemove(char *name)
{
    ...
    if(directory->IsDirectory(name)){
        for(int i = 0; i < directory->getTableSize(); i++){
            if(table[i].isDir){
                int j = sprintf(buff, "%s/%s",pwd,table[i].name);
                recurRemove(buff);
            }
            else{
...
bool FileSystem::Remove(char *name)
{
    ...
    directory = new Directory(NumDirEntries);
    directory->FetchFrom(directoryFile);
    sector = directory->Find(name); //strtok name?
    if (sector == -1)
    {
        delete directory;
        return FALSE; // file not found
